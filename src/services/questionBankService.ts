...
export const generateUniqueQuestions = async (
...
): Promise<{questions: ExamQuestion[], source: 'ai' | 'local', warning?: string}> => {
  // Check if online
  ...
  if (online) {
    try {
      ...
      const warning = aiQuestions.length < count 
        ? `Only ${aiQuestions.length} questions could be generated by the AI.` 
        : undefined;
      
      return { questions: aiQuestions, source: 'ai', warning };
    } catch (error) {
